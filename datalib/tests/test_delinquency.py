...
def test_delinquency_curve__success_case():
    ...
    approval_rate, default_rate = delinquency_curve(y_true, y_scores)
...
def test_calibration_display_compute(iris_data_binary):
    ...
    approval_rate, default_rate = delinquency_curve(y, y_prob)
    ...
    assert (
        viz.ax_.get_xlabel()
        == "Relative percentage of population approved (Positive class: 1)"
    )
    assert viz.ax_.get_ylabel() == "Default Rate (Positive class: 1)"

    expected_legend_labels = ["LogisticRegression"]
